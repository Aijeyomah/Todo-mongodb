...
export const createdOkResponse = (res, data, message) =>
  res.status(201).json({
    status: 201,
    data,
    message,
...
export const pageNotFoundError = (res, message) =>
  res.status(404).json({ status: "error", error_code: 404, message });

export const fieldNotFoundError = (res, message) =>
  res.status(422).json({ status: "error", error_code: 422, message });
